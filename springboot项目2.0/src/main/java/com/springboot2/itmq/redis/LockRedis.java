...
public class LockRedis {
    ...
    public String getRedisLock(Long acquireTimeout, Long timeOut) {
        Jedis conn = null;//jedis就是集成了redis的一些命令操作，封装了redis的java客户端。提供了连接池管理
        ...
        try {
            ...
            // 使用循环方式重试的获取锁
            //System.currentTimeMillis()  系统当前时间毫秒级别
            Long endTime = System.currentTimeMillis() + acquireTimeout;
            //就比如说获取锁之前的超时时间是5秒，当前时间+获取锁之前的时间，等于就是说这个循环只有五秒的循环时间
            while (System.currentTimeMillis() < endTime) {
                // 获取锁
                // 6.使用setnx命令插入对应的redislockKey ，如果返回为1 成功获取锁
                if (conn.setnx(redislockKey, identifierValue) == 1) {
                    // 设置对应key的有效期 expire设置时间
